...
def add_model_selector():
    ...
    st.sidebar.markdown("<h3 class='section-header'>🎤 Voice Settings</h3>", unsafe_allow_html=True)
    st.sidebar.markdown("""
    <div style="background-color:#1E293B; padding:10px; border-radius:5px; border-left:3px solid #60A5FA;">
        <p style="margin-bottom:5px; font-weight:600; color:#E2E8F0;">Current Voice Model:</p>
        <p style="margin:0; color:#94A3B8;">Whisper Large v3</p>
        <p style="margin-top:5px; font-size:0.8rem; color:#94A3B8;">Used for audio transcription</p>
    </div>
    """, unsafe_allow_html=True)
...
def create_fallback_voice_tab():
    """Create a simplified voice tab that uses file upload instead of microphone"""
    st.markdown("<div class='section-header'>🎤 Process Voice Recording</div>", unsafe_allow_html=True)
    
    # Instructions with warning about cloud deployment
    st.markdown("""
    <p style="font-size:1.05rem !important; color:#E2E8F0 !important; background-color:#581C87; padding:15px; border-radius:8px; border-left:4px solid #C084FC;">
        Upload a voice recording file (.mp3, .m4a, .wav) to transcribe and process your question.
    </p>
    """, unsafe_allow_html=True)
    ...
    uploaded_file = st.file_uploader(
        ...
        type=["mp3", "m4a", "wav"],
        help="Upload a voice recording file"
    )
    
    process_button = st.button("🔊 Process Voice Recording", use_container_width=True, disabled=uploaded_file is None)
    ...
    if uploaded_file and process_button:
        ...
        # Return the audio file path for processing
        return temp_path
